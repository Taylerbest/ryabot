...
class IslandMap {
    constructor() {
        ...
        // Параметры для плавного зума БЕЗ ДЕРГАНИЙ
        this.zoomSpeed = 0.25;
        this.minZoom = 0.3;
        this.maxZoom = 8;
        ...
        // Параметры для перемещения карты пальцами
    ...
    // ИСПРАВЛЕННЫЙ зум колесиком БЕЗ ДЕРГАНИЙ
    handleWheelZoom(e) {
        e.preventDefault();
        ...
        // Более плавный зум с фиксированной точкой
        const zoomFactor = e.deltaY > 0 ? 0.9 : 1.1;
        const newZoom = Math.max(this.minZoom, Math.min(this.maxZoom, this.currentZoom * zoomFactor));
        
        // Применяем зум мгновенно БЕЗ анимации, чтобы избежать дерганий
        this.setZoomInstantly(newZoom, mouseX, mouseY);
    }

    // Мгновенная установка зума БЕЗ анимации
    setZoomInstantly(newZoom, centerX, centerY) {
        // Сохраняем текущую позицию относительно точки зума
        const oldZoom = this.currentZoom;
        const scrollLeft = this.container.scrollLeft;
        const scrollTop = this.container.scrollTop;
        
        // Вычисляем точку в координатах канваса
        const canvasX = (centerX + scrollLeft) / oldZoom;
        const canvasY = (centerY + scrollTop) / oldZoom;
        ...
        // Пересчитываем позицию скролла, чтобы точка зума осталась на месте
        const newScrollLeft = canvasX * newZoom - centerX;
        const newScrollTop = canvasY * newZoom - centerY;
        
        this.container.scrollLeft = newScrollLeft;
        this.container.scrollTop = newScrollTop;
    }

    // Touch события - ИСПРАВЛЕНЫ для работы pinch + pan
    handleTouchStart(e) {
        e.preventDefault();
    ...
    handleTouchMove(e) {
        ...
        if (this.isPinching && this.touches.length === 2) {
            ...
            if (Math.abs(scale - 1) > 0.01) {
                ...
                this.setZoomInstantly(newZoom, centerX, centerY);
    ...
    handleTouchEnd(e) {
        ...
        if (this.touches.length === 0) {
            // Если касание было коротким и без перетаскивания - это клик
            if (touchDuration < 300 && this.isDragging && 
                Math.abs(this.touches[0]?.clientX - this.dragStartX || 0) < 10 &&
                Math.abs(this.touches[0]?.clientY - this.dragStartY || 0) < 10) {
                // Обрабатываем как клик по участку
    ...
    // Обработка клика на мобильном устройстве
    handleTouchClick(e) {
    ...
    // Mouse события для перетаскивания на десктопе
    handleMouseDown(e) {
